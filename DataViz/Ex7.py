...
print(trips_freq.shape)
print(trips_freq.head())
trips_freq = trips_freq.pivot(
    index="pickup_community_area",
    columns="dropoff_community_area",
    values="numtrips"
)
#trips_freq = trips_freq.pivot("pickup_community_area", "dropoff_community_area", "numtrips")
trips_freq = trips_freq.replace(np.nan, 0)
trips_freq = trips_freq.astype(int)

fig = plt.figure()

# Change the colormap used to range from yellow to orange to red
